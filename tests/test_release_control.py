...
from test_coordinator import auto_enable_custom_integrations as auto_enable_custom_integrations
...
from custom_components.boiler_flow_control.core.model import ReturnCorrectionState
...
async def test_complete_fresh_zones_infer_only_after_debounce(hass, freezer):
    ...
    assert calls[-1]["value"] == 70
    assert hass.states.get("binary_sensor.boiler_flow_control_dhw_active").state == "on"
...
async def test_icon_is_served_by_home_assistant(hass, hass_client):
    ...
    response = await client.get("/api/brands/integration/boiler_flow_control/icon.png?placeholder=no")
    assert response.status == 200
    icon = Path(__file__).resolve().parents[1] / "custom_components/boiler_flow_control/brand/icon.png"
    assert await response.read() == icon.read_bytes()


async def test_settings_persist_even_when_boiler_unavailable(hass):
    c, _ = await auto(hass)
    ...
    reloaded = BFCStore(hass, c._entry.entry_id)
    await reloaded.async_load()
    assert reloaded.get("enabled") is False and reloaded.get("mode_override") == "hold"
...
async def test_disabled_reload_does_not_resume_auto_writes(hass):
    ...
    restored = c._entry.runtime_data
