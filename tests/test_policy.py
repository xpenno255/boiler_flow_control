...
def test_infer_dhw_relay_signal_always_wins():
    assert infer_dhw_demand(relay_demand_on=True, zone_configured=False, aggregate_demand=None, zone_max=None)
    assert infer_dhw_demand(relay_demand_on=True, zone_configured=True, aggregate_demand=50.0, zone_max=20.0)


def test_infer_dhw_no_zone_list_never_infers():
    assert not infer_dhw_demand(relay_demand_on=False, zone_configured=False, aggregate_demand=100.0, zone_max=None)


def test_infer_dhw_only_charge_signature():
    # aggregate includes DHW: 100 while every zone reads 0 -> inferred DHW-only
    assert infer_dhw_demand(relay_demand_on=False, zone_configured=True, aggregate_demand=100.0, zone_max=0.0)


def test_infer_dhw_below_threshold_not_inferred():
    assert not infer_dhw_demand(relay_demand_on=False, zone_configured=True, aggregate_demand=85.0, zone_max=0.0)


def test_infer_dhw_zone_max_nonzero_not_inferred():
    # a genuine heating demand, not DHW
    assert not infer_dhw_demand(relay_demand_on=False, zone_configured=True, aggregate_demand=100.0, zone_max=15.0)


def test_infer_dhw_all_zones_unavailable_is_not_inferred():
    # cannot confirm zero demand across the zones -> do not infer
    assert not infer_dhw_demand(relay_demand_on=False, zone_configured=True, aggregate_demand=100.0, zone_max=None)
