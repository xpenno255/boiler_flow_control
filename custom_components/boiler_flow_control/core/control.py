...
def effective_target(target: float, minimum: float, maximum: float, step: float = 1.0) -> float | None:
    """Resolve a number's grid and bounds without rounding above a hard cap."""
    if not all(isfinite(v) for v in (target, minimum, maximum, step)) or minimum > maximum or step <= 0:
        return None
...
class DhwDemandTracker:
    ...
    def update(self, relay: bool | None, inferred: bool, definite_off: bool, now: datetime) -> bool:
        if relay is True:
            ...
            self.candidate_since = None
            if definite_off or self.last_confirmed is None or now - self.last_confirmed >= timedelta(minutes=2):
...
class RoomFeedback:
    ...
    def update(self, rooms: dict[str, tuple[float, float]], now: datetime, active: bool) -> tuple[float, float | None]:
        ...
        for entity, (temperature, target) in rooms.items():
            ...
            if elapsed >= 10:
                ...
                if elapsed >= 20:
                    self.samples[entity] = (now - timedelta(minutes=10), temperature - slope * 10, target)
